...
def test_a_comment_never_sticks_out_past_the_diff_it_hangs_under(page, desk):
    branch = page.evaluate("() => data.branches[0].ref")
    desk.post(
        "/comments", [{"branch": branch, "path": "sample.py", "line": FIRST_EDIT, "side": "new", "text": "a remark"}]
    )
    page.reload(wait_until="load")
    ...
    for step in range(10):
        page.evaluate(
            """(wide) => {
              for (const card of document.querySelectorAll('section.file')) card.style.width = `${wide}px`;
            }""",
            900 + step / 10,
        )
...
def test_an_unchanged_answer_leaves_the_page_alone(page, desk):
    ...
    desk.post(
        "/comments",
        [{"branch": branch, "path": "sample.py", "line": FIRST_EDIT, "side": "new", "text": "left by someone else"}],
    )
